...
class App extends react_1.default.Component {
    constructor(props) {
        super(props);
        this.resetGame = () => this.setState({
            ball: Math.round((ROW_SIZE * COL_SIZE) / 2) + ROW_SIZE,
        });
        this.moveBoard = (playerBoard, isUp) => {
            ...
            if (!this.touchingEdge(playerEdge)) {
                ...
                const newDir = (this.state.deltaY !== (COL_SIZE ^ isUp)) ? +this.state.deltaY * -1 : this.state.deltaY;
        ...
        this.touchingEdge = (pos) => (0 <= pos && pos < COL_SIZE) || (COL_SIZE * (ROW_SIZE - 1) <= pos && pos < COL_SIZE * ROW_SIZE);
        this.touchingPaddle = (pos) => {
        ...
        this.isScore = (pos) => (this.state.deltaX === -1 && pos % COL_SIZE === 0) || (this.state.deltaX === 1 && (pos + 1) % COL_SIZE === 0);
        this.moveOpponent = () => {
        ...
        this.touchingPaddleEdge = (pos) => this.state.player[0] === pos ||
    ...
    render() {
        const board = [...Array(ROW_SIZE * COL_SIZE)].map((_, pos) => {
            ...
            return <canvas_1.default key={pos} k={pos} name={val}/>;
        });
