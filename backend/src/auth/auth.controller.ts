...
export class AuthController {
    ...
    @Get('token/:code')
    async getAuthToken(@Param('code') code: string, @Request() request, @Response() response):Promise<void>{
        const dataToPost = {
        ...
        const a = await this.AuthService.OauthSystemCodeToAccess(request, response, dataToPost)
        console.log(a);
        // return a;
    }
}
